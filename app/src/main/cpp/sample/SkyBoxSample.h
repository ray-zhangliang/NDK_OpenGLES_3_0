...
public:
    ...
    virtual void LoadMultiImageWithIndex(int index, NativeImage *pImage) {
        if (pImage) {
            LOGCATE("SkyBoxSample::LoadMultiImageWithIndex pImage = %p, index=%d",
                    pImage->ppPlane[0], index);
    ...
    virtual void Init() {
        ...
        m_ProgramObj = GLUtils::CreateProgram(vSkyBoxShaderStr, fSkyBoxShaderStr, m_VertexShader,
                                              m_FragmentShader);
        if (m_ProgramObj) {
        ...
        m_CubeProgramObj = GLUtils::CreateProgram(vCubeShaderStr, fCubeShaderStr, m_VertexShader,
                                                  m_FragmentShader);
        if (m_CubeProgramObj) {
        ...
        if (!m_TextureId) {
            ...
            for (int i = 0; i < sizeof(m_pSkyBoxRenderImg) / sizeof(NativeImage); ++i) {
                glTexImage2D(
                        ...
                        GL_RGBA, m_pSkyBoxRenderImg[i].width, m_pSkyBoxRenderImg[i].height, 0,
                        GL_RGBA, GL_UNSIGNED_BYTE, m_pSkyBoxRenderImg[i].ppPlane[0]
                );
            }
...
private:
    ...
    GLuint m_CubeProgramObj = GL_NONE;

    GLint m_SamplerLoc = GL_NONE;
    GLint m_MVPMatLoc = GL_NONE;

    GLint m_CubeSamplerLoc = GL_NONE;
    GLint m_CubeMVPMatLoc = GL_NONE;
    GLint m_CubeModelMatLoc = GL_NONE;
    GLint m_ViewPosLoc = GL_NONE;

    GLuint m_CubeVaoId = GL_NONE;
    GLuint m_CubeVboId = GL_NONE;
    GLuint m_SkyBoxVaoId = GL_NONE;
    GLuint m_SkyBoxVboId = GL_NONE;
